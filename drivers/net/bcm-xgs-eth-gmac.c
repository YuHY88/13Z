...
static volatile uint8_t *gmac_regs[BCM_ETH_MAX_PORT_NUM] = {
	(volatile uint8_t *)GMAC0_DEVCONTROL,
	(volatile uint8_t *)GMAC1_DEVCONTROL 
};

static int gmac_disable_dma(struct eth_device *dev, struct eth_dma *dma, int dir);
static int gmac_enable_dma(struct eth_device *dev, struct eth_dma *dma, int dir);
...
static uint32_t dma_ctrlflags(struct eth_device *dev, uint32_t mask, uint32_t flags)
{
	debug("%s enter\n", __func__);
	...
	if (g_dmactrlflags & DMA_CTRL_PEN) {
		...
		control = reg32_read(GMAC_DMA_TX_CTRL_ADDR(dev->index));
		reg32_write(GMAC_DMA_TX_CTRL_ADDR(dev->index), control | D64_XC_PD);
		if (reg32_read(GMAC_DMA_TX_CTRL_ADDR(dev->index)) & D64_XC_PD) {
			/*
			 * We *can* disable it, therefore it is supported;
			 * restore control register
			 */
			reg32_write(GMAC_DMA_TX_CTRL_ADDR(dev->index), control);
		} else {
...
static int dma_tx_init(struct eth_device *dev, struct eth_dma *dma)
{
	dma64dd_t *descp = NULL;
	uint8_t *bufp;
	int i;
	...
	reg32_write(GMAC_DMA_TX_ADDR_LOW_ADDR(dev->index), (uint32_t)(dma->tx_desc_aligned));
	reg32_write(GMAC_DMA_TX_ADDR_HIGH_ADDR(dev->index), 0);

	/* now update the dma last descriptor */
	reg32_write(GMAC_DMA_TX_PTR_ADDR(dev->index), ((uint32_t)(dma->tx_desc_aligned)) & D64_XP_LD_MASK);

	return 0;
}

static int dma_rx_init(struct eth_device *dev, struct eth_dma *dma)
{
	uint32_t last_desc;
	dma64dd_t *descp = NULL;
	uint8_t *bufp;
	...
	reg32_write(GMAC_DMA_RX_ADDR_LOW_ADDR(dev->index), (uint32_t)descp);
	reg32_write(GMAC_DMA_RX_ADDR_HIGH_ADDR(dev->index), 0);

	/* now update the dma last descriptor */
	reg32_write(GMAC_DMA_RX_PTR_ADDR(dev->index), last_desc);

	return 0;
}

static int dma_init(struct eth_device *dev, struct eth_dma *dma)
{
	debug(" %s enter\n", __func__);
	 ...
	 */
	dma_ctrlflags(dev, DMA_CTRL_ROC | DMA_CTRL_PEN, 0);

	debug("rx burst len 0x%x\n",
	      (reg32_read(GMAC_DMA_RX_CTRL_ADDR(dev->index)) & D64_RC_BL_MASK)
	      >> D64_RC_BL_SHIFT);
	debug("tx burst len 0x%x\n",
	      (reg32_read(GMAC_DMA_TX_CTRL_ADDR(dev->index)) & D64_XC_BL_MASK)
	      >> D64_XC_BL_SHIFT);
	...
	dma_ctrlflags(dev, DMA_CTRL_ROC | DMA_CTRL_PEN /* mask */,
		      DMA_CTRL_ROC /* value */);

	return 0;
}

static int dma_deinit(struct eth_device *dev, struct eth_dma *dma)
{
	debug(" %s enter\n", __func__);

	gmac_disable_dma(dev, dma, MAC_DMA_RX);
	gmac_disable_dma(dev, dma, MAC_DMA_TX);
#ifndef ETH_NO_MALLOC
	free(dma->tx_buf);
#endif
	dma->tx_buf = NULL;
...
int gmac_tx_packet(struct eth_device *dev, struct eth_dma *dma, void *packet, int length)
{
...
#endif
	/* now update the dma last descriptor */
	reg32_write(GMAC_DMA_TX_PTR_ADDR(dev->index), last_desc);
...
bool gmac_check_tx_done(struct eth_device *dev, struct eth_dma *dma)
{
	...
	intstatus = reg32_read(GMAC_INT_STATUS_ADDR(dev->index));
	...
	if (intstatus & (I_XI0 | I_XI1 | I_XI2 | I_XI3)) {
		...
		reg32_write(GMAC_INT_STATUS_ADDR(dev->index), intstatus);
	} else {
		debug("Tx int(0x%x)\n", intstatus);
	}
...
int gmac_check_rx_done(struct eth_device *dev, struct eth_dma *dma, uint8_t *buf)
{
	void *bufp, *datap;
	...
	stat0 = reg32_read(GMAC_DMA_RX_STATUS0_ADDR(dev->index)) & D64_RS0_CD_MASK;
	stat1 = reg32_read(GMAC_DMA_RX_STATUS1_ADDR(dev->index)) & D64_RS0_CD_MASK;
	...
	stat0 = reg32_read(GMAC_DMA_RX_STATUS0_ADDR(dev->index));
	stat1 = reg32_read(GMAC_DMA_RX_STATUS1_ADDR(dev->index));
	...
	/* get buffer offset */
	control = reg32_read(GMAC_DMA_RX_CTRL_ADDR(dev->index));
	offset = (control & D64_RC_RO_MASK) >> D64_RC_RO_SHIFT;
	rcvlen = cpu_to_le16(*(uint16_t *)bufp);
	...
	reg32_write(GMAC_DMA_RX_PTR_ADDR(dev->index), ((uint32_t)descp) & D64_XP_LD_MASK);
...
static int gmac_disable_dma(struct eth_device *dev, struct eth_dma *dma, int dir)
{
	...
	if (dir == MAC_DMA_TX) {
		...
		reg32_write(GMAC_DMA_TX_CTRL_ADDR(dev->index), D64_XC_SE);
		SPINWAIT(((status = (reg32_read(GMAC_DMA_TX_STATUS0_ADDR(dev->index)) &
				     D64_XS0_XS_MASK)) !=
			  D64_XS0_XS_DISABLED) &&
		 ...
		 * transfer finishes
		 */
		reg32_write(GMAC_DMA_TX_CTRL_ADDR(dev->index), 0);
		SPINWAIT(((status = (reg32_read(GMAC_DMA_TX_STATUS0_ADDR(dev->index)) &
				     D64_XS0_XS_MASK)) !=
		 ...
		 * transfer finishes
		 */
		reg32_write(GMAC_DMA_RX_CTRL_ADDR(dev->index), 0);
		SPINWAIT(((status = (reg32_read(GMAC_DMA_RX_STATUS0_ADDR(dev->index)) &
				     D64_RS0_RS_MASK)) !=
...
static int gmac_enable_dma(struct eth_device *dev, struct eth_dma *dma, int dir)
{
	...
	if (dir == MAC_DMA_TX) {
		...
		control |= reg32_read(GMAC_DMA_RX_CTRL_ADDR(dev->index)) & D64_RC_BL_MASK;
		control |= HWRXOFF << D64_RC_RO_SHIFT;

		reg32_write(GMAC_DMA_RX_CTRL_ADDR(dev->index), control);
		 ...
		 * set the lastdscr for the rx ring
		 */
		reg32_write(GMAC_DMA_RX_PTR_ADDR(dev->index), ((uint32_t)(dma->rx_desc_aligned) +
			...
			unsigned char reg, unsigned short *value)
{
	...
	*value = readl(GMAC_MII_DATA_ADDR) & 0xffff;
	...
	return 0;
#else
    *value = chip_phy_rd(1, phyaddr, reg);
    debug("MII read data 0x%x\n", *value);
    return 0;
#endif
}
			 ...
			 unsigned char reg, unsigned short value)
{
	...
	return 0;
#else
    debug("MII write phy 0x%x, reg 0x%x, data 0x%x\n",
	      phyaddr, reg, value);
    chip_phy_wr(1, phyaddr, reg, value);
    return 0;
#endif
}

void gmac_init_reset(struct eth_device *dev)
{
	debug("%s enter\n", __func__);

	/* set command config reg CC_SR */
	reg32_set_bits(UNIMAC_CMD_CFG_ADDR(dev->index), CC_SR);
	udelay(GMAC_RESET_DELAY);
}

void gmac_clear_reset(struct eth_device *dev)
{
	debug("%s enter\n", __func__);

	/* clear command config reg CC_SR */
	reg32_clear_bits(UNIMAC_CMD_CFG_ADDR(dev->index), CC_SR);
	udelay(GMAC_RESET_DELAY);
}

static void gmac_enable_local(struct eth_device *dev, bool en)
{
	uint32_t cmdcfg;
	...
	cmdcfg = reg32_read(UNIMAC_CMD_CFG_ADDR(dev->index));
	...
	reg32_write(UNIMAC_CMD_CFG_ADDR(dev->index), cmdcfg);

	/* bring mac out of reset */
	gmac_clear_reset(dev);
	...
	//printf("##### 0x%x = 0x%x \n", UNIMAC_CMD_CFG_ADDR(dev->index), reg32_read(UNIMAC_CMD_CFG_ADDR(dev->index)));

	return;
}

int gmac_enable(struct eth_device *dev)
{
	gmac_enable_local(dev, 1);

	/* clear interrupts */
	reg32_write(GMAC_INT_STATUS_ADDR(dev->index), I_INTMASK);
	return 0;
}

int gmac_disable(struct eth_device *dev)
{
	gmac_enable_local(dev, 0);
	return 0;
}

int gmac_set_speed(struct eth_device *dev, int speed, int duplex)
{
	uint32_t cmdcfg;
	uint32_t hd_ena;
	uint32_t speed_cfg;
    ...
    cmdcfg = reg32_read(GMAC_SERDESCONTROL(dev->index));
    cmdcfg &= ~(SC_FORCE_SPD_STRAP_MASK);
    switch (speed_cfg) {
        case 2:
            cmdcfg |= SC_FORCE_SPD_1G_VAL;
            break;
        case 1:
            cmdcfg |= SC_FORCE_SPD_100M_VAL;
            break;
        default:
            break;
    }
    reg32_write(GMAC_SERDESCONTROL(dev->index), cmdcfg);
    udelay(10);
...
int gmac_set_mac_addr(struct eth_device *dev, unsigned char *mac)
{
	/* set our local address */
	debug("GMAC: %02x:%02x:%02x:%02x:%02x:%02x\n",
	      mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
	reg32_write(UNIMAC_MAC_MSB_ADDR(dev->index), htonl(*(uint32_t *)mac));
	reg32_write(UNIMAC_MAC_LSB_ADDR(dev->index), htons(*(uint32_t *)&mac[4]));
...
gmac_mdio_set(int en)
{
    ...
    tmp = reg32_read((volatile uint32_t *)IPROC_WRAP_MISC_CONTROL);
...
static void
gmac_serdes_init(struct eth_device *dev)
{
    /* bring serdes out of reset */
    uint32_t sdctl, sdstat0, sdstat1;

    sdctl = reg32_read(GMAC_SERDESCONTROL(dev->index));
    sdstat0 = reg32_read(GMAC_SERDESSTATUS0(dev->index));
    sdstat1 = reg32_read(GMAC_SERDESSTATUS1(dev->index));
    printf("et%d: %s() serdes_status0: 0x%x; serdes_status1: 0x%x\n", dev->index, __FUNCTION__, sdstat0, sdstat1);
    ...
    reg32_write(GMAC_SERDESCONTROL(dev->index), sdctl);
    ...
    sdstat0 = reg32_read(GMAC_SERDESSTATUS0(dev->index));
    sdstat1 = reg32_read(GMAC_SERDESSTATUS1(dev->index));
    //printf("##### et%d: %s() sdctl: 0x%x; serdes_status0: 0x%x; serdes_status1: 0x%x\n", dev->index, sdctl, sdstat0, sdstat1);
...
gmac_core_reset(void)
{
    ...
    reg32_write((volatile uint32_t *)(AMAC_IDM0_IDM_RESET_CONTROL),0);
    reg32_write((volatile uint32_t *)(AMAC_IDM1_IDM_RESET_CONTROL),0);

    tmp = reg32_read((volatile uint32_t *)(AMAC_IDM0_IO_CONTROL_DIRECT));
    ...
    reg32_write((volatile uint32_t *)(AMAC_IDM0_IO_CONTROL_DIRECT),tmp);

    tmp = reg32_read((volatile uint32_t *)(AMAC_IDM1_IO_CONTROL_DIRECT));
    ...
    reg32_write((volatile uint32_t *)(AMAC_IDM1_IO_CONTROL_DIRECT),tmp);
    ...
    tmp |= (1 << AMAC_IDM0_IO_CONTROL_DIRECT__CT);
    tmp |= (1 << AMAC_IDM0_IO_CONTROL_DIRECT__BYPASS_CT);
    tmp |= (0xB << AMAC_IDM0_IO_CONTROL_DIRECT__ARCACHE_R);
    tmp |= (0x7 << AMAC_IDM0_IO_CONTROL_DIRECT__AWCACHE_R);
    tmp |= (1 << AMAC_IDM0_IO_CONTROL_DIRECT__CLK_EN);
...
int gmac_mac_init(struct eth_device *dev)
{
...
#if (defined(CONFIG_HELIX4) || defined(CONFIG_KATANA2) || defined(CONFIG_SABER2))
    /* Enable ENA_EXT_CONFIG when there is serdes in between */
	cmdcfg |= CC_AE;
#endif
	/* put mac in reset */
	gmac_init_reset(dev);
	...
	reg32_set_bits(GMAC_DEV_CTRL_ADDR(dev->index), DC_MROR);
	/* PHY: set smi_master to drive mdc_clk */
	reg32_set_bits(GMAC_PHY_CTRL_ADDR(dev->index), PC_MTE);

	/* clear persistent sw intstatus */
	reg32_write(GMAC_INT_STATUS_ADDR(dev->index), 0);

	if (dma_init(dev, dma) < 0) {
	...
	reg32_write(GMAC_INTR_RECV_LAZY_ADDR(dev->index), 1 << GMAC_IRL_FRAMECOUNT_SHIFT);
	...
	reg32_write(UNIMAC_FRM_LENGTH_ADDR(dev->index), PKTSIZE + 32);
...
int gmac_add(struct eth_device *dev)
{
    ...
    if (dev->index >= CONFIG_GMAC_NUM) {
		error("%s: wrong dev_num %d!\n", __func__, dev->index);
		return -1;
    }
#endif
#ifndef ETH_NO_MALLOC
    ...
    dma->tx_desc = (void *)alloc_ptr;
    dma->tx_desc_aligned = (void *)(alloc_ptr & (~0xf));
	debug("TX Descriptor Buffer: %p; length: 0x%x\n",
	      dma->tx_desc_aligned, sizeof(dma64dd_t) * TX_BUF_NUM);
    alloc_ptr = (uint32_t)(dma->tx_desc_aligned + (sizeof(dma64dd_t) * TX_BUF_NUM));
    dma->tx_buf = (uint8_t *)alloc_ptr;
	debug("TX Data Buffer: %p; length: 0x%x\n",
	      dma->tx_buf, TX_BUF_SIZE * TX_BUF_NUM);
    alloc_ptr = (uint32_t)(dma->tx_buf + (TX_BUF_SIZE * TX_BUF_NUM));
    alloc_ptr += 0x10;
    dma->rx_desc = (void *)alloc_ptr;
	dma->rx_desc_aligned = (void *)(alloc_ptr & (~0xf));
    debug("RX Descriptor Buffer: %p, length: 0x%x\n",
	      dma->rx_desc_aligned, sizeof(dma64dd_t) * RX_BUF_NUM);
    alloc_ptr = (uint32_t)(dma->rx_desc_aligned + (sizeof(dma64dd_t) * RX_BUF_NUM));
    dma->rx_buf = (uint8_t *)alloc_ptr;
	debug("RX Data Buffer: %p; length: 0x%x\n",
	      dma->rx_buf, RX_BUF_SIZE * RX_BUF_NUM);
#endif
	g_dmactrlflags = 0;
	/* changed by lihz - 2018.9.6 */
	eth->phy_interface = PHY_INTERFACE_MODE_SGMII;//PHY_INTERFACE_MODE_GMII;
